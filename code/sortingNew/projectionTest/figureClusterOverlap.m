...
if length(spikes1)==0 || length(spikes2)==0
...
if mode==1
	...
	subplot(3,2,1)
        bar(bc1, fhat1,  'FaceColor', colors{1});
	hold on
	...
	subplot(3,2,3)
        h=bar(bc2, fhat2,  'FaceColor', colors{2});
	...
	subplot(3,2,2);
	plot( 1:size(spikes1,2), spikes1(1:10:end,:)','color', colors{1});
    hold on
    plot(1:size(spikes2,2), spikes2(1:10:end,:)','color', colors{2});
	...
	subplot(3,2,4);
	plot( 1:size(spikes1Orig,2), spikes1Orig(1:10:end,:)','color', colors{1});
    hold on
    plot(1:size(spikes2Orig,2), spikes2Orig(1:10:end,:)','color', colors{2});
