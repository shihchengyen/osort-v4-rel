...
%
%HM Edit: Added filteredsignal as input
function [newSpikes,newTimestamps, shifted, indsToKeep] = realigneSpikes(allSpikes, allTimestamps, alignParam, stdEstimate, filteredSignal, peakAlignMethod)
if nargin<=5
    peakAlignMethod=1;
end
...
%         currentSpike = [currentSpike(diff:maxPos) currentSpike(maxPos+1:end) currentSpike(end)*ones(1,diff-1)];
        currentSpike = [currentSpike(diff+1:maxPos) currentSpike(maxPos+1:end) currentSpike(end)*ones(1,diff)]; % HM Edit
        changed=true;
...
shifted = shifted(indsToKeep); % HM Edit
