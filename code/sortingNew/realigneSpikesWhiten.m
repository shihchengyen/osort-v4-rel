...
%kaminskij/ 3 october 2013. new clone function (realigneSpikes) for realignment of decorrelated spikes.
%This function search for peaks in time 50:150 (arbitrary) instead 50:200 and does not drop 
%spike to original position if realignment is longer than shouldMax/2. 
%This change does not affect peak of power method 

 
%
function [newSpikes,newTimestamps, shifted] = realigneSpikesWhiten(allSpikes, allTimestamps, alignParam, stdEstimate,peakAlignMethod)
if nargin<=4
    peakAlignMethod=1;
end
...
for i=1:size(allSpikes,1)
    ...
    switch(peakAlignMethod)
        case 1 %normal
            maxPos=0;
%             maxPos = findPeak( currentSpike(50:150), stdEstimate, alignParam );
            maxPos = findPeak( currentSpike(50:200), stdEstimate, alignParam ); % HM Edit
%             maxPos=maxPos+50;
            if maxPos ~= -1
                maxPos = maxPos+50-1; % HM Edit
            end
        case 3 %peak power
...
%     
    ...
    if changed && diff<shouldMax/2   % HM Edit
        newSpikes(i,:) = currentSpike;
    else
        display(['Not realigned: ' num2str(i)]); % HM Edit
