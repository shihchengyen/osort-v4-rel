...
function [assigned, nrAssigned, baseSpikes, baseSpikesID] = sortSpikesOnline( spikes, stdEstimate, sortTill, thresholdMethod, Cinv, transformedSpikes,runningAverageLength )
if nargin<=4
    thresholdMethod=1;
    Cinv=[];
    transformedSpikes=[];
    runningAverageLength=100;%kaminskij
end
...
nrBaseSpikes=1;			% how many base spikes are known. HM Edit - to include the cluster that the first spike belongs to
...
%TODO testing
if nrDatapoints==256
    if thresholdMethod == 1 % Approximate threshold
        weights(1:20)=0;
        weights(201:256)=0;
    else
...
%             end
            ...
            %runningAverageLength=100; kaminskij - commented out 4 October 2013 -  now runningAverageLength if given as a function parameter 
            % HM EDIT - removed this runningAverageLength bit so that base
            % spike gets updated with all spikes, not just 100 of them
