...
%             if abs(minimum/maximum)>1.5 || abs(maximum/minimum)>1.5
%                 doMinMax=true;
%             end
            %realign according to order of peak (first try max ....min)
            if find(spikeSignal==maximum) < find(spikeSignal==minimum)
                peakInd = find( spikeSignal == maximum ); % If max comes first
            else
                % then min...max
                peakInd = find( spikeSignal == minimum );
            end
        else
            % if only one peak is significant (e.g. only minimum is significant -> realign at minimum)
...
if doMinMax % Takes whichever peak is larger (in cases of 1. if 1 peak is dominant 2. if only 1 peak is significant)
