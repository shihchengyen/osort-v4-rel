function varargout = DefineUsableClustersGUI(varargin)
% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @DefineUsableClustersGUI_OpeningFcn, ...
                   'gui_OutputFcn',  @DefineUsableClustersGUI_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
channelnumber = [];
% HM Edit - to find where figure name starts after the threshold value
firstchar = find(isstrprop(string,'alpha'),1);
for i = firstchar:length(string); % HM Edit changed index starting from 1 to starting from first char
...
function DefineUsableClustersGUI_OpeningFcn(hObject, eventdata, handles, varargin)
...
function varargout = DefineUsableClustersGUI_OutputFcn(hObject, eventdata, handles) 
...
%         filename = [PATH(1:(index(end)-1)) 'final' '\' FULLNAMES{value}];
        filename = [PATH FULLNAMES{value}]; % HM Edit - suppressed the previous two lines
    end
...
if isempty(strfind(FULLNAMES{value}, 'selected'))
    ...
    filename = [PATH(1:(index-1)) 'final' '/' FULLNAMES{value}]; % HM Edit - changed 'final\' to 'final/'
end
...
global FIGUREPATH
sortdirindex = strfind(PATH, 'sort'); % HM Edit
FIGUREPATH = [PATH(1:sortdirindex-1),'figs',THRESH,'/']; % HM EDit to define path for figures
defineUsableClusters_forGUI(FIGUREPATH,cluster,channel1,channel2);
